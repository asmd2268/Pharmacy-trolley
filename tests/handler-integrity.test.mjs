...
test('architecture modules load before the main application script', () => {
  const main = html.indexOf('<script src="./app.js"></script>');
  assert.ok(main > 0, 'main application script should exist');
...
test('main application does not define duplicate named functions', () => {
  ...
  for (const match of app.matchAll(/\bfunction\s+([A-Za-z_$][\w$]*)\s*\(/g)) {
